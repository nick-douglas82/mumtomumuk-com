...
Class WpApiEvent
{
    ...
    {
        $found = Event::where('wp_id', $data->id)->first();
    ...
    {
        if ( empty($data->acf->event_at) || empty($data->acf->event_end) ) {
            return;
        }

        echo "Create Event" . $data->id . " ";
        $post              = new Event;
        ...
        $post->event_at    = $data->acf->event_at;
        $post->event_end   = $data->acf->event_end;
    ...
    {
        echo "Update Event" . $data->id . " ";

        if (empty($data->acf->event_at) || empty($data->acf->event_end)) {
            return;
        }

        $post = Event::find($foundPost->id);
        $post->delete();
        $post->seo()->delete();
