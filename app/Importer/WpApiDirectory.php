...
Class WpApiDirectory
{
    ...
    {
        $found = Directory::on($this->location)->where('wp_id', $data->id)->first();
    ...
    protected function deletePost($posts)
    {
        foreach ($posts as $post) {
            $post = Directory::on($this->location)->find($post);
    ...
    {
        // IDs in API
        $posts    = Directory::on($this->location)->pluck('wp_id');
    ...
    {
        echo "Update Directory" . $data->id . " ";
        $post = Directory::find($foundPost->id);
        $post->delete();
        $post->seo()->delete();
